...
class IAPAuth(httpx.Auth):
    ...
    def __init__(  # nosec
            self, client_id: str, service_account: typing.Union[GoogleServiceAccount, dict],
        ...
        if jwt_soft_expiration > 3600:
            raise ValueError("Parameter `jwt_soft_expiration` should NOT be larger than 3600 seconds (1 hour)!")
    ...
    def sync_auth_flow(self, request: httpx.Request):
        ...
        if not self.google_iap_jwt or self.is_jwt_expired:
            with self._sync_lock as _, httpx.Client() as client:
                self.google_iap_jwt = self.service_account.get_bearer_token(target_audience=self.client_id,
                                                                            client=client,
    ...
    async def async_auth_flow(self, request: httpx.Request):
        ...
        if not self.google_iap_jwt or self.is_jwt_expired:
            async with self._async_lock as _, httpx.AsyncClient() as client:
                self.google_iap_jwt = await self.service_account.async_get_bearer_token(target_audience=self.client_id,
                                                                                        client=client,
                                                                                        grant_type=self.jwt_bearer_token_grant_type, )
    ...
    def is_jwt_expired(self):
        ...
        if (jwt.decode(self.google_iap_jwt, options={"verify_signature": False})[
                "iat"] + self.jwt_soft_expiration) < time.time():
            return True
